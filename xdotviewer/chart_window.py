...
from ros_element import ElementType, NodeType, RosElement, ChartRequest, ChartType, ChartValue
...
class ChartWindow(Gtk.Window):
    def __init__(self, r2ta: R2TAInterface, node: RosElement):
        self.r2ta = r2ta
        self.node = node

        Gtk.Window.__init__(self)
        self.set_title(' '.join(unquote(node.name).split("&")))
        ...
        scatter_icon = Gtk.Image.new_from_file("xdotviewer/media/scatter.png")
        scatter = Gtk.ToolButton(label="Scatter plot", icon_widget=scatter_icon)
        scatter.connect("clicked", lambda w: self.set_and_rerun("chart", ChartType.SCATTER))
        toolbar.insert(scatter, -1)

        histogram_icon = Gtk.Image.new_from_file("xdotviewer/media/histogram.png")
        histogram = Gtk.ToolButton(label="Histogram", icon_widget=histogram_icon)
        histogram.connect("clicked", lambda w: self.set_and_rerun("chart", ChartType.HISTOGRAM))
    ...
    def render(self):
        return self.r2ta.render(ChartRequest(node=self.node.name, value=self.value, plot=self.chart, bins=self.bins))
    ...
    def export_as(self, w):
        ...
        if chooser.run() == Gtk.ResponseType.OK:
            ...
            chooser.destroy()
            self.r2ta.export(filename, self.node.name, self.value)
        else:
            chooser.destroy()
    ...
    def export_file(self, filename, format_):
        if format_ == "jpg":
            ...
            with open(self.image_path) as f:
                s = f.read()
                svg2jpg(bytestring=s, write_to=filename)
        elif format_ == "png":
            from cairosvg import svg2png
            with open(self.image_path) as f:
                s = f.read()
