...
class R2TAInterface:
    ...
    def render(self, chart: ChartRequest) -> str:
        ...
        try:
            args = [self.tracer_cmd, "chart", "--element-id", chart.node, '--property', chart.value.value, '-i', self.data_dir, '-o', outdir, '--size', str(chart.size), chart.plot.value]
    ...
    def export(self, outfile: str, node: str, value: ChartValue):
        try:
            args = [self.tracer_cmd, "extract", "--element-id", node, '--property', value.value, '-i', self.data_dir, '-o', outfile]
