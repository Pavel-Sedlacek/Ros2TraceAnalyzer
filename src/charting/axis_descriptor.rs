...
use crate::argsv2::{chart_args::ChartVariants, extract_args::AnalysisProperty};
    ...
    pub fn best_fit(&self, value: i64) -> AxisBestFit {
        match &self.quantity {
            ...
            AxisQuantity::SimpleSi { base, .. } => {
                println!("Expressing {base} {value}");
                AxisBestFit::SimpleSi { 
                    base: *base,
                    target: *SiPrefix::all()
                        .iter()
                        .map(|d| (d, d.express_value(value, *base)))
...
pub enum AxisQuantity {
    ...
    SimpleSi { name: &'static str, base: SiPrefix }
}

impl AxisQuantity {
    pub fn to_best_fit(&self) -> AxisBestFit {
        match &self {
            AxisQuantity::Duration { base } => AxisBestFit::Duration { base: *base, target: *base },
            AxisQuantity::SimpleSi { base, .. } => AxisBestFit::SimpleSi { base: *base, target: *base },
        }
    }
}

pub enum AxisBestFit {
    Duration { base: DurationUnit, target: DurationUnit },
    SimpleSi { base: SiPrefix, target: SiPrefix }
}

impl AxisBestFit {
    pub fn convert(&self, value: i64) -> f64 {
        match self {
            AxisBestFit::Duration { base, target } => {
                target.express_value(value, *base)
            },
            AxisBestFit::SimpleSi { base, target } => {
                target.express_value(value, *base)
            },
        }
    } 
    ...
    match chart_variant {
        ChartVariants::Histogram(_) => match charted_value {
            AnalysisProperty::CallbackDuration => AxisDescriptors {
                x: AxisDescriptor {
                    label: "Duration",
                    quantity: AxisQuantity::Duration { base: DurationUnit::Nanosecond }
                },
                y: AxisDescriptor {
                    label: "Samples",
                    quantity: AxisQuantity::SimpleSi { name: "", base: SiPrefix::Base }
                },
            },
            &AnalysisProperty::ActivationsDelay => AxisDescriptors { 
                x: AxisDescriptor { label: "Delay", quantity: AxisQuantity::Duration { base: DurationUnit::Nanosecond } }, 
                y: AxisDescriptor { label: "Activations", quantity: AxisQuantity::SimpleSi { name: "", base: SiPrefix::Base } }
            },
            AnalysisProperty::PublicationsDelay => AxisDescriptors { 
                x: AxisDescriptor { label: "Delay", quantity: AxisQuantity::Duration { base: DurationUnit::Nanosecond } }, 
                y: AxisDescriptor { label: "Publications", quantity: AxisQuantity::SimpleSi { name: "", base: SiPrefix::Base } }
            },
            AnalysisProperty::MessagesDelay => AxisDescriptors { 
                x: AxisDescriptor { label: "Delay", quantity: AxisQuantity::Duration { base: DurationUnit::Nanosecond } }, 
        ...
        ChartVariants::Scatter => match charted_value {
            ...
            AnalysisProperty::ActivationsDelay => AxisDescriptors { 
                x: AxisDescriptor { label: "Nth Activation", quantity: AxisQuantity::SimpleSi { name: "", base: SiPrefix::Base } }, 
                y: AxisDescriptor { label: "Delay", quantity: AxisQuantity::Duration { base: DurationUnit::Nanosecond } }
            },            
            AnalysisProperty::PublicationsDelay => AxisDescriptors { 
                x: AxisDescriptor { label: "Nth Publication", quantity: AxisQuantity::SimpleSi { name: "", base: SiPrefix::Base } }, 
                y: AxisDescriptor { label: "Delay", quantity: AxisQuantity::Duration { base: DurationUnit::Nanosecond } }
            },
            AnalysisProperty::MessagesDelay => AxisDescriptors { 
