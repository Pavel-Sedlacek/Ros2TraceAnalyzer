...
use plotters::{chart::{ChartBuilder, ChartContext}, coord::types::RangedCoordi64, prelude::{Cartesian2d, DrawingBackend, IntoLogRange, LogCoord, Rectangle}, style::Color};
    ...
    pub fn new(histogram_data: &HistogramData, data: &ChartableData, axis_descriptor: &AxisDescriptors) -> Self {
        ...
        let (bin_width, x_range) = {
            ...
            if raw_range == 0 {
                ...
                let mut x_end = if max % bin_width == 0 { max } else { (max.div_euclid(bin_width) + 1) * bin_width };
        ...
        let data: std::collections::HashMap<u64, i64> = data
            .iter()
            .into_group_map_by(|&v| {
                ((v - min) / bin_width).min(data_bins as i64 - 1)
            })
            .iter()
...
impl ChartData<Coords> for HistogramChart {
    fn draw_into<'a, B: DrawingBackend>(&self, canvas: &mut ChartBuilder<B>) -> Result<ChartContext<'a, B, Coords>, ChartConstructionError> {
        let mut context = canvas.build_cartesian_2d(self.x_range.0..self.x_range.1, (self.y_range.0..self.y_range.1).log_scale())
            .map_err(|e| ChartConstructionError::InvalidCoordinateSystem(e.to_string()))?;


        context.draw_series(
            self.data.iter().map(|(b, size)| {
