...
impl BinarySQLStore {
    pub fn new(sqlite_file: std::path::PathBuf) -> Result<BinarySQLStore, BinarySQLStoreError> {
        ...
        if brand_new {
            sqlite_connection.execute("DROP TABLE IF EXISTS blobs", ())
                .map_err(|e| BinarySQLStoreError::SQLiteError(e))?;
            
            sqlite_connection.execute("CREATE TABLE blobs(
    ...
    pub fn write(&self, name: &str, data: impl serde::Serialize) -> Result<(), BinarySQLStoreError> {
    ...
    pub fn read<T: for<'a> serde::Deserialize<'a>>(&self, name: &str) -> Result<T, BinarySQLStoreError> {
        let mut statement = self.sqlite_connection.prepare(
            "SELECT data FROM blobs WHERE blob_name = ?1 LIMIT 1", 
        ).map_err(|e| BinarySQLStoreError::SQLiteError(e))?;
