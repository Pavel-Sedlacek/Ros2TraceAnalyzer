...
impl AnalysisOutput for MessageLatency {
    ...
    fn get_binary_output(&self) -> impl Serialize {
        self.calculate_stats().into_iter().map(Into::<MessageLatencyExport>::into).collect::<Vec<_>>()
    }  
}
...
impl From<MessageLatencyStats> for MessageLatencyExport {
    fn from(value: MessageLatencyStats) -> Self {
        let target_node = value.subscriber.lock().map( |s|
            s.get_node().map(|v| get_node_name_from_weak(&v.get_weak()).unwrap_or("Unknown".to_string()))
        ).map(|v| v.to_string()).unwrap_or("Unknown".into());

        let source_node = value.publisher.map(|p| 
            p.lock().map( |s|
                s.get_node().map(|v| get_node_name_from_weak(&v.get_weak()).unwrap_or("Unknown".to_string()))
            ).map(|v| v.to_string()).unwrap_or("Unknown".into())
